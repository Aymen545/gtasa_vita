...
- To have the cut songs back you'll need to extract the `STREAMS` audio files with saat 1.10 by alci [SAAT 1.10 by alcy](https://web.archive.org/web/20070305050639/http://pdescobar.home.comcast.net:80/gta/saat/SAAT_release_1_10.zip) from your downgraded steam/rockstar launcher version or from the disc version that has the cut songs in the root of the `saat` folder:
  ...
  - Format factory works too with a sample rate of 22050, a bit rate of 128kbps, audio channel set to 1 and VBR to 0, also put output to source file folder.
  - On soundforge or another audio converter with 56kbps and 22050hz mono highest quality convert them (can try higher max would be 128kbps if you want to test it) from `.ogg` to `.mp3` and make sure you uncheck the id3 settings so the tracks wouldn't have any id3 tags on them.
  - Then after the conversion compress the converted `.mp3` files into `.zip` files for each folder meaning `CH`, `DS`, `RJ`...etc , and compression set to store with 7z or rar.
  - After that rename the file extension from `.zip` to `.osw` for each one of them but leave their original name untouched so they'll be like this: `DS.osw`, `CH.osw`...etc.
